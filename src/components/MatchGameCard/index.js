...
class MatchGameCard extends Component {
  state = {
    activeTabId: tabsList[0].tabId,
    imagesList: initialImagesList,
    score: 0,
    randomImageDetails: initialImagesList[0],
    timerInSeconds: 60,
    isTimerRunning: true,
  }
  ...
  onClickThumbnail = thumbnailId => {
    const {randomImageDetails} = this.state
    const {id} = randomImageDetails
    if (thumbnailId === id) {
      this.setState(
        prevState => ({
  ...
  generateRandomImg = () => {
    ...
    this.setState({
      randomImageDetails: randomlyGeneratedImage,
    })
  }

  renderRandomImage = () => {
    const {randomImageDetails} = this.state
    const {imageUrl} = randomImageDetails
    return <img src={imageUrl} alt="match" className="match-image" />
  }
  ...
  renderGameOver = () => {
    ...
    return (
      <div className="game-over-container">
        ...
        <h1 className="resultant-score">{score}</h1>
        <button
  ...
  render() {
    const {score, timerInSeconds} = this.state
    return (
      <>
        <Header score={score} timerInSeconds={timerInSeconds} />
        <div className="main-game-container">{this.renderGameView()}</div>
      </>
    )
  }
}
